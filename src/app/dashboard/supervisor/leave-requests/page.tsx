...
export default function SupervisorLeaveRequestsPage() {
    ...
    try {
      ...
      if (data.success) {
        ...
        setRequests(prev => prev.map(r =>
          r.id === id
            ? { ...r, supervisorApproval: action, finalStatus: action === "REJECTED" ? "REJECTED" : r.finalStatus }
            : r
        ));
        setSummary(s => ({
          ...s,
  ...
  const filtered = useMemo(() =>
    requests.filter(r => filter === "ALL" || r.supervisorApproval === filter),
    [requests, filter]
  );
  ...
  return (
    <div className="lr-page">
      ...
      <div className="lr-header">
        <div>
          <h1><Calendar size={22} /> Leave Requests</h1>
          <p>Dashboard / Supervisor / Leave Requests</p>
        </div>
      ...
      <div className="lr-summary">
        <div className="lr-stat lr-stat-blue">
          <Users size={18} />
          <div><h3>{summary.total}</h3><p>Total</p></div>
        </div>
      ...
      <div className="lr-tabs">
        {(["ALL", "PENDING", "APPROVED", "REJECTED"] as FilterType[]).map(f => (
          <button
            key={f}
            className={`lr-tab ${filter === f ? "active" : ""}`}
            onClick={() => setFilter(f)}
          >
            {f === "ALL" ? "All" : f.charAt(0) + f.slice(1).toLowerCase()}
            {f !== "ALL" && (
              <span className={`lr-tab-count lr-count-${f.toLowerCase()}`}>
                {f === "PENDING" ? summary.pending : f === "APPROVED" ? summary.approved : summary.rejected}
              </span>
            )}
          </button>
        ))}
      ...
      {/* LOADING */}
      {loading && (
        <div className="lr-loading"><Loader2 size={20} className="spin" /> Loading requests...</div>
      )}
            ...
            <tbody>
              {filtered.map(req => (
                <>
                  <tr
                    key={req.id}
                    className={`lr-row ${req.supervisorApproval !== "PENDING" ? "lr-row-reviewed" : ""}`}
                  >
                    <td>
                      <div className="lr-worker">
                        <span className="lr-worker-name">{req.workerName}</span>
                        <span className="lr-worker-meta">
                          {req.employeeCode && <span>{req.employeeCode}</span>}
                          {req.department && <span>{req.department}</span>}
                        </span>
                      </div>
                    </td>
                    <td>
                      <span className="lr-leave-type">{req.leaveType}</span>
                      {req.hasDocument && (
                    ...
                    <td>
                      <div className="lr-duration">
                        <span className="lr-days">{req.totalDays}d</span>
                        <span className="lr-dates">{fmt(req.startDate)} — {fmt(req.endDate)}</span>
                      </div>
                    </td>
                    <td>{fmt(req.appliedOn)}</td>
                    <td>
                      <span className={`lr-status-badge lr-status-${req.supervisorApproval.toLowerCase()}`}>
                        {req.supervisorApproval === "PENDING"  && <Clock size={11} />}
                        {req.supervisorApproval === "APPROVED" && <CheckCircle2 size={11} />}
                        {req.supervisorApproval === "REJECTED" && <XCircle size={11} />}
                        {req.supervisorApproval.charAt(0) + req.supervisorApproval.slice(1).toLowerCase()}
                      </span>
                    </td>
                    <td>
                      {req.supervisorApproval === "PENDING" ? (
                        ...
                        <div className="lr-detail-panel">
                          ...
                          <div className="lr-detail-inline">
                            <div className="lr-detail-item">
                              ...
                              <span className={`lr-status-badge lr-status-${req.managerApproval.toLowerCase()}`}>
                                {req.managerApproval.charAt(0) + req.managerApproval.slice(1).toLowerCase()}
                              </span>
                            </div>
                            <div className="lr-detail-item">
                              <span className="lr-detail-label">Final Status</span>
                              <span className={`lr-status-badge lr-status-${req.finalStatus.toLowerCase()}`}>
                                {req.finalStatus.charAt(0) + req.finalStatus.slice(1).toLowerCase()}
                              </span>
                            </div>
                          </div>
                        </div>
